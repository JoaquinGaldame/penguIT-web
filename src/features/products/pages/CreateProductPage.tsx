...
export function CreateProductPage() {
  ...
  function getCreateProductErrorMessage(
    error: unknown,
  ): string {
    if (
      typeof error === 'object' &&
    ...
    return 'No pudimos crear el producto. Intentá nuevamente.';
  }
  ...
  const createProduct = async (
    ...
    try {
      ...
      setSubmitError(
        getCreateProductErrorMessage(error),
      );
    }
  };
  ...
  return (
    <Stack spacing={3}>
      <Box>
        <Breadcrumbs sx={{ mb: 1 }}>
          <Link component="button" onClick={returnToProducts}>Productos</Link>
