...
export function RecipeListPage() {
  ...
  const { search, category, status, costStatus, selectedRecipeId, detailsOpen } =
    useAppSelector((state) => state.recipes);
  const [searchInput, setSearchInput] = useState(search);
  const debouncedSearch = useDebouncedValue(searchInput, 300);
  const { data, isError, isFetching, isLoading, refetch } = useGetRecipesQuery();
  ...
  const filteredRecipes = useMemo(() => {
    ...
    return recipes.filter((recipe) => {
      if (category !== 'all' && recipe.productCategory !== category) return false;
      ...
      return (
        ...
        PRODUCT_CATEGORY_LABELS[recipe.productCategory]
          ...
          .includes(term) ||
        RECIPE_STATUS_LABELS[recipe.status].toLocaleLowerCase('es').includes(term)
      );
    });
  ...
  return (
    <Stack spacing={2.5}>
      ...
      >
        <Box>
          <Typography component="h1" variant="h4" sx={{ mb: 0.75 }}>Recetas</Typography>
      ...
      {isLoading ? (
        <Box sx={{ display: 'grid', gap: 1.5, gridTemplateColumns: { xs: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(4, minmax(0, 1fr))' } }}>
          {Array.from({ length: 4 }).map((_, index) => (
            <Skeleton key={index} variant="rounded" height={82} animation="wave" />
          ))}
        </Box>
      ) : (
      ...
      <Box sx={{ minHeight: 400 }}>
        {isFetching && !isLoading && <LinearProgress aria-label="Actualizando recetas" />}

        {hasRefetchError && (
          <Alert severity="warning" role="status" sx={{ mb: 2 }}>
            No pudimos actualizar las recetas. Se mantienen visibles los últimos datos disponibles.
          </Alert>
        )}
